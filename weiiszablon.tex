...
Aby skutecznie zarządzać połaczeniem BLE z urządzeniem pośredniczącym w zaprojektowanej aplikacji utworzono moduł, w postaci klasy opartej o wzorzec Singleton, korzystający z biblioteki Flutter Blue Plus[]. Utworzona klasa udostępnia szereg typowych metod dla zarządznia połączeniem.

Odnajdź dokumentację FlutterBluePlusa:
\begin{enumerate}[label=\alph*), leftmargin=1.25cm]
   \item initCheckingBleAdapter()
   \item checkLocationEnabled()
   \item scanBleDevices()
   \item sendIrCodeToDevice()
   \item wymyślenie jak nadmienić że są też inne funkcje i czy w ogóle pomijać 
\end{enumerate}
\subsection{Ekran łączenia się z urządzeniem pośredniczącym poprzez BLE}
Przejście kolejno przez elementy łączenia się od init state do rozłączania.

Pokazywanie stanów aplikacji i małych wycinków kodu ekranu.
\subsection{Moduł obsługi przechowywania modelu telewizora}
Opisanie sharedPrefs ogólnie i pokazanie funkcji jako przykładu save i get. Opis hex validatora.
...
Przedstawienie funkcjonalności ekranu i przyjętych rozwiązań. tutaj pokazanie 1 przycisku że wysyłamy dane
\clearpage
