...
Aby uzyskać możliwość połaczenia wielu użytkowników z serwerem urządzenia pośredniczącego metoda wywoływana podczas każdego połączenia nowego użytkownika musi wznowić rozgłaszanie dostępności serwera. Ta funkcjonalność została zaimplementowana w funkcjach wywwoływanych podczas zdarzeń serwera.
\subsection{Moduł interpretujący dane z BLE}
Aby ułatwić pracę z danymi odebranymi przez urządznie pośredniczące utworzono moduł przetwarzający je. Zawiera on wygodny typ wylieczniowy wykorzystywany do nadania przejrzystych nazwy programowych dla odebranych typów wciśniętego przycisku pilota z aplikacji mobilnej.
Z kolei klasa Controller odpowiada za interpretację danych odebranych za pomocą charakterystyk serwera BLE. Korzysta ona z instancji klasy Connection aby pobrać dane charakterystyk i je przetowrzyć. Aby to zrobić, klasa ta udostępnia metody pozwalające odświeżać przechowywane w polach tej klasy ostatnio odebrane dane od aplikacji mobilnej. Publicznie dostępne elementy modułu znajdują się na List. \ref*{Lst:controller}.
...
Dane odebrane za pomocą charakterystyk są przechowywane w postaci bitowej. Aby odczytać ich wartości należy zinterpretować ich zawartość za pomocą odpowiedniego typu danych. Metoda updateButtonType() z klasy Controller interpretuje dane charakterystki za pomocą typu 8 bitowej liczby całkowitej, ponieważ ilość przycisków jest niewielka. Odbywa się tam też prosta kontrola poprawności danych, poprzez sprawdznie czy odebrane dane mieszczą się w zakresie zdefiniowanych przycisków pilota. Kod źródłowy tej funkcji znajduje sie na List. \ref*{Lst:dataProcessing}.
...
Autorski moduł obsługi ekranu OLED jest w rzeczywistości nakładką na klasę sterującą wyświetlaczem OLED z biliotek firmy Adafruit. Realizowane jest to dzięki dziedziczeniu publicznym utworzonej klasy Display z klasy Adafruit\_SH1106G. Sama nakładka jak i dwa poprzednie znalazła się w przestrzeni nazw bt2ir.

Utworzony obiekt stara się udostępnić metody, dzięki którym użytkownik w prosty i przejrzysty sposób może rysować konkretne obrazy i napisy na wyświetlaczu, w postaci przygotowanych i zparametryzowanych ekranów. Przygotowane zostały ekrany takie jak przedstawienie liczby połączonych smartfonów do serwera BLE i jego stanu, wyświetlanie komunikatu o odebraniu wciśniętego przycisku z aplikacji mobilnej, czy wyświetlenia odebranego, za pomocą modułu odbiorczego z diodą, sygnału IR. Wszytkie dostępne funkcjonalności utworzonej nakładki w postaci metod publicznych zostały przedstawione na List. \ref*{Lst:graphics}.
...
Prywatna logika tej nakładki opiera się o 4 metody prywatne. Dwie z nich rysują bardziej podstawowe elementy ekranów, takie jak rysowanie wyrównanego tekstu czy wielkich cyfr. Dzięki nim powstały także dwie metody z nich korzystające, przy pomocy których, można narysować na wyświetlaczu ikonę lub cyfrę z podpisem. Tak wydzielone fragmenty logiki kodu, przyspieszyły i ułatwiły zbudowanie wszystkich publicznie dostępnych funkcjonalności. Prywatnie zadeklarowana lista prywatnych metod utworzonej nakładki znajduje się na List. \ref*{Lst:graphicsPrivate}.
...
Ikony rysowane na ekranach przez publiczne metody nakładki zostały umieszczone w programie za pomocą tablic bajtów, a same tablice zostały wygenerowane poprzez progowanie binarne obrazów w odcieniach szarości przy pomocy aplikacji internetowej image2cpp[]. Po przekonwertowaniu obrazów na tablice bajtów i usunięciu z niepotrzebnych wygenerowanych części kodu, można ją umieścić w oprogramowaniu i wykorzystywać do wyświetlania obrazu, którego ta tablica dotyczy na ekranie OLED. Jedna z wygenerowanych tablic znajduje się na List. \ref*{Lst:imageArray}.
...
Dzięki utworzonym narzędziom budowano publiczne metody dostępne dla użytkownika nakładki. W każdej takiej funkcji najpierw czyszczony był wyświetlacz, następnie rysowano odpowiedni obraz na podstawie zdefiniowanych tablic bajtów wraz z wyśrodkowanym podpisem na ekranie w buforze, by końcowo wyświetlić dane zgromadzone w buforze rysowania. Jedną z tych metod jest drawMoveUp rysująca komunikat dla użytkownika o wciśnięciu przycisku przejścia w górę na pilocie w aplikacji mobilnej. Jej implementacja została przedstawiona na List. \ref*{Lst:drawMoveUp}, a jej fizyczny efekt pokazano na Rys. \ref*{Fig:drawMoveUp}.
...
\begin{figure}[ht]
   \centering
   \includegraphics[width=12cm]{images/drawMoveUp.jpg}
...
Aplikacja mobilna została przygotowana przy pomocy frameworka Flutter. To narzędzie programistyczne udostępnia wygodny sposób dzielenia interfejsu użytkownika na ekrany i został on wykorzystany w aplikacji pilota. Wydzielono w projekcie główną stronę aplikacji odpowiedzialną za wyświetlanie jej nazwy, czyli ,,Bluetooth2IR'', oraz nawigowanie pomiędzy utworzonymi ekranami funkcjonalnymi. Elementy interfejsu zarządzane przez nią zostały wskazane na Rys. \ref*{Fig:mainPageElements}. Strona ta, zaimplementowana w pliku homepage.dart jest głównym widgetem utworzonej aplikacji uruchamianej w main.dart. Plik uruchamiający aplikację oprócz samego jej uruchomienia definiuje także domyślne style czcionki, kolorów, przycisków i tła.
\begin{figure}[ht]
   \centering
...
\begin{enumerate}[label=\alph*), leftmargin=1.25cm]
   \item Ekran ,,Connection'' - działający jako panel łączenia się aplikacji pilota z urządzeniem pośredniczącym
   ...
   \item Ekran ,,Main buttons'' - udostępniający różne podstawowe przyciski pilota
   \item Ekran ,,Numeric'' - udostępniający przyciski cyfr pilota
\end{enumerate}

\subsection{Moduł obsługujący BLE}
Aby skutecznie zarządzać połaczeniem BLE z urządzeniem pośredniczącym w zaprojektowanej aplikacji utworzono moduł, w postaci klasy opartej o wzorzec Singleton, korzystający z biblioteki Flutter Blue Plus\cite{flutterBluePlus}. Utworzona klasa udostępnia szereg typowych metod dla zarządznia połączeniem.

Aby nawiązać połączenie z urządzeniem pośredniczącym, najpierw należy odnaleźć wszystkie dostępne urządzenia w okolicy. Przed tym, jednak jeszcze trzeba się upewnić, że smartfon posiada włączony adapter BLE. W projektowanej klasie służy do tego metoda initCheckingBleAdapter przedstawiona na List. \ref*{Lst:checkBLE}. Dodaje ona subskyrpcję do strumienia sprawdzającego włączenie adaptera BLE, która jeśli wykryje, że adapter jest wyłączony pyta jednokrotnie użytkownika o chęć włączenia go.
...
Mając dostępny sposób na sprawdzenie działania lokalizacji i adaptera BLE, można przystąpić do skanowania dostępnych urządzeń bezprzewodowych. Zadanie to pełni metoda scanBleDevices() przedstawiona na List. \ref*{Lst:scanBLE}. Sprawdza ona najpierw dostępność lokalizacji, a następnie dodaje subskrypcje do strumienia dostępnych urządzeń zawierających w nazwie "Bluetooth2IR for TV". Następnie sprawdzane są flagi dostępności wcześniej sprawdzonych przez inne funkcje zasobów i jeśli wszytskie są dostępne, następuje 3 sekundowe skanowanie dostępnych urządzeń.
...
Zakładając, że odnaleziono przynajmniej 1 urządzenie pośredniczące, klasa udostępnia możliwość połączenia się z nim za pomocą metody connectToBleDevice(), która sprawdza czy urządzenie posiada odpowiednie charakterystyki odbioru informacji o wciśniętym przycisku na pilocie. Realizowane jest to za pomocą sprawdzania UUID serwisu i charakterystyk. Jeśli każdy z atrybutów urządzenia pośredniczącego zgadza się z wymaganiami systemu, urządzenie pozostaje z nim połączone (już samo sprawdzanie atrybutów BLE wymaga chwilowego nawiązania połączenia z tym urządzeniem).
Posiadając nawiązane połączenie moduł jest gotowy do wysyłania informacji o wciśniętym przycisku. Za wysył danych do urządzenia pośredniczącego odpowiadają 2 metody, jedna z nich to sendIrCodeToDevice() zaprezentowana na List. \ref*{Lst:sendingBLE}. Wysyła ona do charakterystyki kodu IR przycisku, dane w postaci listy 8 bajtowych liczb całkowitych bez znaku, czekając na odpowiedź o powodzeniu lub niepowodzeniu transmisji.
...
Przełączając się na ten ekran lub włączając aplikację, gdyż jest on domyślnym pod ekranem, użytkownik uruchomi sprawdzenie włączenia adaptera BLE oraz lokalizacji. Stanie się tak, ponieważ zaimplementowane zostało automatyczne uruchamianie skanowania, które wymaga wymienionych zasobów do poprawnego działania, a metoda inicjalizacji tego obiektu zajmuje się sprawdzeniem ich dostępności. Ekrany przedstawiające zapytania o włączenie lokalizacji oraz modułu Bluetooth, a także ekran podczas wyszukiwania dostępnych urządzeń zostały przedstawione na Rys. \ref*{Fig:initBLE}.
\begin{figure}[ht]
   \centering
   \includegraphics[width=12cm]{images/initBLE.png}
...
Jeśli użytkownik włączył swoje urządzenie pośredniczące i jest ono w pobliżu, a także w aplikacji nastąpiło poprawne skanowanie, zobaczy on swoje urządzenie na liście pod nazwą ,,Bluetooth2IR for TV''. Może się połączyć on z tak odnalezionym urządzeniem za pomocą kliknięcia w nie na liście, a później jeśli chce, rozłączyć za pomocą przytrzymania tego elementu. Ekran przedstawiający odnalezione urządzenie znajdujące się na liście dostępnych oraz jego stan po połączeniu się z tym urządzeniem znajdują się na Rys. \ref*{Fig:connectBLE}.

\begin{figure}[ht]
   \centering
   \includegraphics[width=10cm]{images/connectBLE.png}
...
W aplikacji mobilnej pilota uniwersalnego użtykownik ma możliwość wyboru modelu telewizora, którym chce sterować. Aby reprezentować programowo zbiór zdefiniowanych modeli telewizorów  i zrealizować zapamiętywanie tego zestawu pomiędzy uruchomieniem aplikacji utworzono moduł tv\_model.dart. Odpowiedzalny jest on za zdefiniowanie klasy daych pojedynczego przycisku pilota, oraz składającej się z takich przycisków klasy pełnego modelu pilota. 

Klasa Button odpowiadająca za pojedynczy przycisk zawiera 2 metody fromJson() i toJson() pozwalające na pracę z danymi zapisanymi w postaci tekstu w formacie JSON. Iplementacja tych metod znajduje się na List. \ref*{Lst:buttonJson}.
...
Z kolei klasa TVModel składa się z wielu obiektów klasy Button z odpowiednio zdefniowanymi typami przycisku w postaci numerycznej oraz nazwy tego modelu telewizora, którym definiowane przysciski na pilocie mają sterować. Posiada ona także metody toJson() oraz fromJson(), które są ciągami wywołań tego rodzaju metod z klasy Button. [jakoś może szerzej to opisać]

Chcąc teraz przechowywać dane pomiędzy uruchomieniami aplikacji można skorzystać z nieulotnej pamięci aplikacji, do której można uzyskać dostęp za pomocą obiektu SharedPreferences udostępnianego przez framework Flutter. Dla wygody i zkonsolidowania operacji wykonywanych na tym obiekcie utoworzono klasę SharedPrefs służącą jako interfejs dostępu do danych zapisanych w pamięci oraz udostępniający łatwe w użyciu metody zapisujące i odczytujące dane zestawów przycisków dla modeli telewizorów. Klasa ta przechowuje instancję obiektu SharedPreferences oraz udostępnia statyczne metody pozwalające zapisywać i odczytywać nawet całe listy zestawów przycisków. Część definicji tej klasy zawierającą pole przechowujące instancę obiektu SharedPreferences, metodę inicjalizującą ten obiekt oraz metody zapisujące i odczytujące listy zestawów przycisków zawarto na List. \ref*{Lst:sharedPrefs}
...
Przedstawienie funkcjonalności ekranu i przyjętych rozwiązań. GG 700 linijek
ale przejdzie się tak samo od uruchomienia do zapisania modelu
\subsection{Ekrany przycisków pilota}
Aby użytkownik mógł finalnie skorzystać ze sterowania telewizorem, udostępnione zostały 2 ekrany. Pierwszy z nich o nazwie ,,Main Buttons'' zawiera najczęściej spotykane typy przycisków jak włączający telewizor, zmieniajacy głośność, kanału i im podobne. Przyciski na tym ekranie zostały rozmiesczone tak aby jak najbardziej imitować ich rozmieszczenie na zwykłym pilocie. Wygląd tego ekranu został przedstawiony na Rys. \ref*{Fig:mainButtons}.
\begin{figure}[ht]
   \centering
   \includegraphics[width=8cm]{images/mainButtons.png}
   \caption{Ekran ,,Main Buttons'' aplikacji mobilnej zawierający popularne przyciski}
   \label{Fig:mainButtons}
\end{figure}

Utworzony został również ekran pozwalający na wprowadzanie cyfr o nazwie ,,Numeric''. Zbudwany został tak, aby cyfry były jak największe na ekranie, a zarazem przyciski pozostały wygodne w użyciu. Jego wygląd w aplikacji przedstawiony został na Rys. \ref*{Fig:numeric}.

\begin{figure}[ht]
   \centering
   \includegraphics[width=8cm]{images/numeric.png}
   \caption{Ekran ,,Numeric'' aplikacji mobilnej zawierający przyciski numeryczne}
   \label{Fig:numeric}
\end{figure}
